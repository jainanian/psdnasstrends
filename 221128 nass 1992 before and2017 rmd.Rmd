...
graphnass_orderLU <- nassvarname_align %>% 
 ...
 graphnass_empty <- crossing(
   descripfacet = graphnass_out$descripfacet %>% unique(), 
   year = c(graphnass_out$year %>%  min(), graphnass_out$year %>%  max())
 ) %>% 
   mutate(
     emptytext = ""
   )
...
graphnass_gg <- graphnass_out%>% 
   ...
   geom_line(
    ...
    facet_wrap(~descripfacet, labeller = label_wrap_gen(width=60)) + 
    theme_minimal() +
